        ...
        stages {
            ...
            stage('deploy') {
                steps {
                    ...
                    //sh 'sudo docker exec -it docker-spark-cluster-master_spark-master_1 /bin/bash -c './bin/spark-submit --class org.example.StreamingJob --master spark://localhost:7077 --packages org.apache.spark:spark-sql-kafka-0-10_2.12:3.0.0 spark-demo-1.0-SNAPSHOT-jar-with-dependencies.jar''
                }
            }
        }
}
